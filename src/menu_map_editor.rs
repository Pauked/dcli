...
fn open_map_editor_from_pwad_id(pwad_id: i32) -> Result<String, eyre::Report> {
...
pub fn open_from_pick_pwad() -> Result<String, eyre::Report> {
...
pub fn update_map_editors() -> Result<String, eyre::Report> {
    let app_settings = db::get_app_settings()?;
    ...
    let db_map_editors = db::get_map_editors()?;
        ...
        match existing_map_editor {
            Some(existing) => {
                ...
                if existing.version != selection.version {
                    ...
                    db::update_map_editor_version(existing.id, &selection.version)?;
                }
            }
            None => {
                db::add_map_editor(&selection)?;
...
pub fn delete_map_editor() -> Result<String, eyre::Report> {
    let map_editor_list = db::get_map_editors()?;
    if map_editor_list.is_empty() {
